...
void main() {
    ...
    vec4 colTemp = vec4(gradientMagnitude, normalizedDirection.x, normalizedDirection.y, 1.0);
    float luminance = dot(colTemp.rgb, W);

    gl_FragColor = vec4(vec3(luminance), colTemp.a);
